...
def is_video(url):
    ...
    """
    return url.lower().endswith(('.mp4', '.webm', '.ogg', '.ogv', '.avi', '.wmv'))
...
def read_file(file_path):
    ...
    with open(file_path, 'r') as f:
        ...
        return LoaderResult(
            buffer = f.read(),
            successful = True,
            metadata = dict(size=stats.st_size, updated_at=datetime.datetime.utcfromtimestamp(stats.st_mtime))
        )


# def download_video(url, length):
...
#
#
# def load_video(context, url, callback, normalize_url_func):
#     url = normalize_url_func(url)
#     f = download_video(url, 1024 * 1024)
#
#
#     partial(return_contents, url=url, callback=callback, context=context, req_start=start)
